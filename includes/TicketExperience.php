...
final class TicketExperience
{
    ...
    public function register(): void
    {
        ...
        add_action('wp_ajax_dizzy_tm_ticket_details', [$this, 'details']);
        add_action('wp_ajax_nopriv_dizzy_tm_ticket_details', [$this, 'details']);
    }
    ...
    public function details(): void
    {
        ...
        if (! preg_match('/^[a-f0-9]{64}$/', $code)) {
            wp_send_json_error(['message' => __('Invalid ticket.', 'dizzy-ticket-manager')], 400);
        }

        $ticket = $this->repository->ticketByCode($code);

        if ($ticket === null || ($ticket['status'] ?? '') !== 'valid') {
            wp_send_json_error(['message' => __('This ticket is not available.', 'dizzy-ticket-manager')], 404);
        }

        global $wpdb;
        $ticketName = (string) $wpdb->get_var($wpdb->prepare(
            "SELECT i.ticket_name FROM {$wpdb->prefix}dizzy_tm_ticket_order_items i INNER JOIN {$wpdb->prefix}dizzy_tm_tickets t ON t.order_item_id=i.id WHERE t.ticket_code=%s LIMIT 1",
            $code
        ));
        ...
        $ticketUrl = add_query_arg('dizzy_tm_paid_ticket', $code, home_url('/'));
    ...
    {
        $order = sanitize_text_field(wp_unslash((string) ($_GET['dizzy_tm_order'] ?? '')));
        ...
        $statusMessage = match ($paymentStatus) {
            'failed' => __('The payment failed. Please try again.', 'dizzy-ticket-manager'),
            'canceled', 'cancelled' => __('The payment was canceled.', 'dizzy-ticket-manager'),
            'expired' => __('The payment session expired. Please start a new payment.', 'dizzy-ticket-manager'),
            default => __('Your payment is being processed. Please wait a moment and refresh the page.', 'dizzy-ticket-manager'),
        };
        ?>
        <style>
            .dizzy-ticket-overlay{align-items:center;background:rgba(0,0,0,.82);display:none;inset:0;justify-content:center;padding:18px;position:fixed;z-index:999999}.dizzy-ticket-overlay.is-open{display:flex}.dizzy-ticket-modal{background:#191919;border:0px solid #292929;border-radius:0;box-shadow:0 24px 70px rgba(0,0,0,.55);box-sizing:border-box;color:#fff;max-height:92vh;max-width:720px;overflow:auto;padding:38px;position:relative;text-align:center;width:100%}.dizzy-ticket-modal h2,.dizzy-ticket-modal h3,.dizzy-ticket-modal p{color:inherit}.dizzy-ticket-close{background:transparent!important;border:0!important;color:#fff!important;font-size:28px!important;line-height:1!important;padding:8px!important;position:absolute;right:1px;top:1px}.dizzy-ticket-modal img{background:#fff;height:auto;max-width:280px;padding:12px;width:72%}.dizzy-ticket-modal .woocommerce-notices-wrapper{margin:0}.dizzy-ticket-modal .woocommerce-message{align-items:stretch;background:#1d1d1d;border:0;color:#fff;display:grid;gap:14px;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));margin:0;padding:24px;text-align:left}.dizzy-ticket-modal .woocommerce-message-text{grid-column:1/-1;margin-bottom:4px;text-align:center}.dizzy-ticket-modal .woocommerce-message::before{display:none}.dizzy-ticket-modal .woocommerce-message-text{line-height:1.6}.dizzy-ticket-modal .woocommerce-message .button.wc-forward{background:#fff;border:1px solid #fff;border-radius:0;color:#050505;box-sizing:border-box;font-size:12px;font-weight:700;letter-spacing:1.5px;padding:16px 28px;text-align:center;text-transform:uppercase;width:100%}.dizzy-ticket-modal .woocommerce-message.is-single-ticket{align-items:center;display:flex;flex-direction:column;justify-content:center}.dizzy-ticket-modal .woocommerce-message.is-single-ticket .woocommerce-message-text{width:100%}.dizzy-ticket-modal .woocommerce-message.is-single-ticket .button.wc-forward{max-width:190px;width:100%}.dizzy-ticket-actions{display:flex;flex-wrap:wrap;gap:10px;justify-content:center;margin-top:22px}.dizzy-ticket-actions button,.dizzy-ticket-actions a{border:1px solid #fff;border-radius:0;box-sizing:border-box;font-size:12px;font-weight:700;letter-spacing:1.2px;padding:14px 22px;text-decoration:none;text-transform:uppercase}.dizzy-ticket-primary{background:#fff!important;color:#050505!important}.dizzy-ticket-back{background:transparent!important;color:#fff!important}.dizzy-ticket-meta{line-height:1.7;margin:18px 0}.dizzy-ticket-loader{padding:45px 10px}@media(max-width:600px){.dizzy-ticket-modal{padding:38px 18px 24px}.dizzy-ticket-modal .woocommerce-message{grid-template-columns:1fr;text-align:center}.dizzy-ticket-modal .woocommerce-message .button.wc-forward,.dizzy-ticket-actions>*{width:100%}}
        ...
        <div class="dizzy-ticket-overlay" id="dizzy-ticket-overlay" role="dialog" aria-modal="true" aria-labelledby="dizzy-ticket-heading">
            <div class="dizzy-ticket-modal"><button type="button" class="dizzy-ticket-close" aria-label="<?php esc_attr_e('Close', 'dizzy-ticket-manager'); ?>">&times;</button><div id="dizzy-ticket-modal-content"></div></div>
        </div>
        <script>
        (()=>{const success=document.querySelector('.dizzy-ticket-success');if(!success)return;const overlay=document.getElementById('dizzy-ticket-overlay'),content=document.getElementById('dizzy-ticket-modal-content'),close=overlay.querySelector('.dizzy-ticket-close'),ajax=<?php echo wp_json_encode(admin_url('admin-ajax.php')); ?>;const initial=success.cloneNode(true);const open=()=>{overlay.classList.add('is-open');document.body.style.overflow='hidden';close.focus()};const shut=()=>{overlay.classList.remove('is-open');document.body.style.overflow='';};const showSuccess=()=>{content.innerHTML='';content.append(initial.cloneNode(true));content.querySelectorAll('a[href*="dizzy_tm_paid_ticket"]').forEach(link=>{link.classList.add('dizzy-ticket-primary');link.addEventListener('click',e=>{e.preventDefault();load(link.href)})});open()};const load=async href=>{const code=new URL(href,location.href).searchParams.get('dizzy_tm_paid_ticket');content.innerHTML='<div class="dizzy-ticket-loader"><?php echo esc_js(__('Loading ticket…', 'dizzy-ticket-manager')); ?></div>';open();try{const body=new URLSearchParams({action:'dizzy_tm_ticket_details',ticket_code:code||''}),response=await fetch(ajax,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body}),json=await response.json();if(!json.success)throw new Error(json.data?.message||'Ticket unavailable');ticket(json.data)}catch(error){content.innerHTML='<h2><?php echo esc_js(__('Ticket could not be loaded.', 'dizzy-ticket-manager')); ?></h2><p>'+String(error.message)+'</p><div class="dizzy-ticket-actions"><button class="dizzy-ticket-back" type="button"><?php echo esc_js(__('Back', 'dizzy-ticket-manager')); ?></button></div>';content.querySelector('button').onclick=showSuccess}};const ticket=data=>{content.innerHTML='<h2 id="dizzy-ticket-heading"><?php echo esc_js(__('Your Ticket', 'dizzy-ticket-manager')); ?></h2><h3></h3><p class="dizzy-ticket-date"></p><img alt="<?php echo esc_js(__('Ticket QR code', 'dizzy-ticket-manager')); ?>"><div class="dizzy-ticket-meta"></div><p><?php echo esc_js(__('Present this QR code at the entrance.', 'dizzy-ticket-manager')); ?></p><div class="dizzy-ticket-actions"><button type="button" class="dizzy-ticket-primary dizzy-save-ticket"><?php echo esc_js(__('Save Ticket', 'dizzy-ticket-manager')); ?></button><button type="button" class="dizzy-ticket-back"><?php echo esc_js(__('Back', 'dizzy-ticket-manager')); ?></button></div>';content.querySelector('h3').textContent=data.event;content.querySelector('.dizzy-ticket-date').textContent=data.date;const img=content.querySelector('img');img.src=data.qr;content.querySelector('.dizzy-ticket-meta').textContent=data.type+' · '+data.holder+' · '+data.code;content.querySelector('.dizzy-ticket-back').onclick=showSuccess;content.querySelector('.dizzy-save-ticket').onclick=()=>save(data,img)};const save=(data,img)=>{if(!data.qr||!img.complete){location.assign(data.url);return}const canvas=document.createElement('canvas');canvas.width=1080;canvas.height=1600;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,1080,1600);ctx.fillStyle='#111827';ctx.textAlign='center';ctx.font='700 54px sans-serif';wrap(ctx,data.event.toUpperCase(),540,150,900,68);ctx.font='36px sans-serif';ctx.fillText(data.date,540,300);ctx.drawImage(img,260,390,560,560);ctx.font='700 38px sans-serif';ctx.fillText(data.type,540,1040);ctx.font='32px sans-serif';ctx.fillText(data.holder,540,1110);ctx.font='28px monospace';ctx.fillText(data.code,540,1180);ctx.font='28px sans-serif';ctx.fillText('<?php echo esc_js(__('Present this QR code at the entrance.', 'dizzy-ticket-manager')); ?>',540,1335);const a=document.createElement('a');a.download=data.filename||'event-ticket.jpg';a.href=canvas.toDataURL('image/jpeg',.95);a.click()};const wrap=(ctx,text,x,y,max,line)=>{const words=String(text).split(/\s+/);let row='';for(const word of words){const test=row?row+' '+word:word;if(ctx.measureText(test).width>max&&row){ctx.fillText(row,x,y);row=word;y+=line}else row=test}if(row)ctx.fillText(row,x,y)};close.onclick=shut;overlay.addEventListener('click',e=>{if(e.target===overlay)shut()});document.addEventListener('keydown',e=>{if(e.key==='Escape')shut()});success.style.display='none';showSuccess()})();
        </script>
        <script>
        (()=>{const content=document.getElementById('dizzy-ticket-modal-content');if(!content)return;const styleNotice=()=>{const result=content.querySelector('.dizzy-ticket-success');if(!result)return;const wrapper=document.createElement('div'),message=document.createElement('div'),text=document.createElement('span'),links=[...result.querySelectorAll('a[href*="dizzy_tm_paid_ticket"]')];wrapper.className='woocommerce-notices-wrapper';message.className='woocommerce-message'+(links.length===1?' is-single-ticket':'');message.setAttribute('role','alert');message.tabIndex=-1;text.className='woocommerce-message-text';text.textContent=result.querySelector('strong')?.textContent||'Payment received. Your tickets are ready.';message.append(text);links.forEach((link,index)=>{link.className='button wc-forward';link.textContent=links.length>1?'OPEN TICKET '+(index+1):'OPEN TICKET';message.append(link)});wrapper.append(message);result.replaceWith(wrapper)};styleNotice();new MutationObserver(styleNotice).observe(content,{childList:true,subtree:true})})();
        </script>
        <?php
    }
}
