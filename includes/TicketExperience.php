...
final class TicketExperience
{
    ...
    public function render(): void
    {
        ...
        <script>
        (()=>{const content=document.getElementById('dizzy-ticket-modal-content');if(!content)return;const styleNotice=()=>{const result=content.querySelector('.dizzy-ticket-success');if(!result)return;const wrapper=document.createElement('div'),message=document.createElement('div'),text=document.createElement('span'),links=[...result.querySelectorAll('a[href*="dizzy_tm_paid_ticket"]')];wrapper.className='woocommerce-notices-wrapper';message.className='woocommerce-message';message.setAttribute('role','alert');message.tabIndex=-1;text.className='woocommerce-message-text';text.textContent=result.querySelector('strong')?.textContent||'Payment received. Your tickets are ready.';message.append(text);links.forEach((link,index)=>{link.className='button wc-forward';link.textContent=links.length>1?'OPEN TICKET '+(index+1):'OPEN TICKET';message.append(link)});wrapper.append(message);result.replaceWith(wrapper)};styleNotice();new MutationObserver(styleNotice).observe(content,{childList:true,subtree:true})})();
        </script>
        <?php
    }
}
